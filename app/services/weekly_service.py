...
from app.services.time_service import is_week_end, user_today, week_start_date


def current_week_start(user: User, today: date | None = None) -> date:
    return week_start_date(today or user_today(user.timezone))
...
def should_offer_weekly(
    ...
    current = today or user_today(user.timezone)
